...
| SC7                        | LOW        | [#118](https://github.com/billchurch/webssh2_client/issues/118) + [PR #119](https://github.com/billchurch/webssh2_client/pull/119) + [PR #123](https://github.com/billchurch/webssh2_client/pull/123) | **Fixed** — `no-unsafe-*` (PR #119) + `strict-boolean-expressions` at error, 214 violations fixed (PR #123, assumed merged to main) |
...
- **SC7** — type-aware ESLint wired for `client/src`; the five `no-unsafe-*` rules enabled at error and the 10 violations fixed (`.eslintrc.json`, `lib/xterm-solid/`) in PR #119 (lint/typecheck/tests green). `strict-boolean-expressions` ratchet completed in [PR #123](https://github.com/billchurch/webssh2_client/pull/123) (assumed merged to main 2026-06-10): all 214 remaining violations fixed across 36 files with behavior-preserving explicit comparisons, rule flipped to **error**, full `check:all` + 318 tests green. Org-policy TypeScript lint rules now fully enforced; #118 closed.
...
### SC7. TypeScript `no-unsafe-*` / `strict-boolean-expressions` not enabled (LOW) — [#118](https://github.com/billchurch/webssh2_client/issues/118) — FIXED in [PR #119](https://github.com/billchurch/webssh2_client/pull/119) + [PR #123](https://github.com/billchurch/webssh2_client/pull/123)

> **Resolved 2026-06-10:** Type-aware linting wired for `client/src`
> (`parserOptions.project`), the five `no-unsafe-*` rules enabled at error and
> their 10 violations fixed in PR #119 (merged 2026-06-10).
> `strict-boolean-expressions` completed in PR #123 (assumed merged to main):
> all 214 violations across 36 files fixed with behavior-preserving explicit
> comparisons (nullable strings → `!= null && !== ''`, nullable booleans →
> `=== true`, `||` fallbacks kept via explicit ternaries, `any` conditionals
> narrowed via `typeof`), rule ratcheted to **error** with default options —
> no `allowNullable*` exemptions, no `eslint-disable` comments. Verified with
> `check:all` (lint, both typechecks, 318 tests) and a production build.
...
| SC7 | Supply-chain | LOW        | **Fixed** [PR #119](https://github.com/billchurch/webssh2_client/pull/119) + [PR #123](https://github.com/billchurch/webssh2_client/pull/123) (assumed merged to main) — type-aware lint, `no-unsafe-*` + `strict-boolean-expressions` at error, 224 violations fixed ([#118](https://github.com/billchurch/webssh2_client/issues/118)) |
